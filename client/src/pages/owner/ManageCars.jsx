...
const {isOwner, axios, currency} = useAppContext();
  ...
  const deleteCar = async (carId) => {
    try {
      ...
      const confimed = window.confirm('Are you sure you want to delete this car?')
     ...
     className='px-4 pt-10 md:px-10 w-full'>
      ...
      <div className='max-w-3xl w-full rounded-md overflow-hidden border border-borderColor mt-6'>
        <table className='w-full border-collapse text-left text-sm text-gray-600 '>
         ...
         <tbody>
            { cars.map((car, index) => (
              <tr key={index} className='bottom-t border-borderColor'>

                <td className='p-3 flex items-center gap-3'>
                  <img src={car.image} alt='' className='h-12 w-12 aspect-square rounded-md object-cover'/>
                  <div className='max-md:hidden'>
                    <p className='font-medium'>{car.brad} {car.model}</p>
                ...
                <td className='flex items-center p-3'>
                  <img onClick={()=>toggleCarAvailability(car._id)} 
                   src={car.isAvaliable ? assets.eye_close_icon : assets.eye_icon} className='cursor-pointer'
                  />
